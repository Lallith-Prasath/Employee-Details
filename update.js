class UserTable {
    ...
    async fetchUsers() {
        try{
            const response = await fetch('https://65a8cad5219bfa3718679868.mockapi.io/prct/employee', { method: "GET" });
    ...
    async addUser() {
        ...
        if(!regex.test(name)|| !regex.test(department)|| !name || name.trim() === "" || !department || department.trim() === "") {
           ...
           try {
              ...
              if(response.ok) {
                 ...
                 const table = document.getElementById('userTableContainer').getElementsByTagName('table')[0];
    ...
    async updateUser(user,row) {
        ...
        let newDepartment = prompt("Enter new department for the user:", user.department);
 
        let regex = /^[a-zA-Z ]+$/;
 
        if(regex.test(newName) || newName === null || newName.trim() === "" || regex.test(newDepartment) || newDepartment === null || newDepartment.trim() === "") {
            alert("No updates made to the user. User inputs are not alphabetical characters.");
            return;
        }
 
        try {
            const response = await fetch(`https://65a8cad5219bfa3718679868.mockapi.io/prct/employee/${user.id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json"
                },
            ...
            if(response.ok) {
                ...
                document.getElementById(`department-${user.id}`).textContent = newDepartment;
                alert("User updated!");
            } else {
                throw new Error(`Error: ${response.statusText}`);
            }
    ...
    async deleteUser(user, row) {  
        let confirmation = confirm(`Are you sure you want to delete the user ${user.name} ?`);
...
const table = UserTable.getInstance('https://65a8cad5219bfa3718679868.mockapi.io/prct/employee');
table.renderTable();
